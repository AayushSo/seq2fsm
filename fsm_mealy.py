...
def get_list(s,enco_type='def'):
	...
	if not enco_type in valid_enco :
		print(f"//Not a valid encoding format \'{enco_type}\'. Taking default")
		enco_type = 'def'
...
def verilog(s,enco_type='def',input_wire='in',state ='state',next_state='next',output_wire = 'out' ,clock_name = 'clk', edge = 'p', reset = 'rst'):
	print("//Mealy FSM Implementation")
	clock_edge = 'posedge ' if (edge =='p' or edge =='posedge') else 'nedgedge '
	...
	if enco_type != 'onehot' :
		...
		if '11' in g:
			for j,i in enumerate(g):
				if i =='11' : print('(',state,'==',y[j][0],') |',end='')
		if '10' in g:
			print('~',input_wire,end='&(')
			for j,i in enumerate(g):
				if i =='10' : print('(',state,'==',y[j][0],') |',end='')
			print('\b)',end='|')
		if '01' in g:
			print(input_wire,end='&(')
			for j,i in enumerate(g):
				if i =='01' : print('(',state,'==',y[j][0],') |',end='')
			print('\b)',end='|')
		print('\b;')
	else :
		print ('assign ',output_wire,' = ',end = '')
		if '11' in g:
			for j,i in enumerate(g):
				if i =='11' : print(state,'[',index(y[j][0]),'] |',end='')
		if '10' in g:
			print('~',input_wire,end='&(')
			for j,i in enumerate(g):
				if i =='10' : print(state,'[',index(y[j][0]),'] |',end='')
			print('\b)',end='|')
		if '01' in g:
			print(input_wire,end='&(')
			for j,i in enumerate(g):
